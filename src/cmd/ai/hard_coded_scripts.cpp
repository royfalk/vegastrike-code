...
class EvadeLeftRightC : public FlyByWire
{
    ...
    Vector facing;
    bool   dir;
public: EvadeLeftRightC( bool updown )
    {
        this->updown = updown;
        facing = Vector( 0, 0, 0 );
...
public:
    ...
    LoopAround( bool aggressive, bool afterburn, bool force_afterburn, int seed ) : FaceTargetITTS( false, 3 )
        , m( Vector( 0, 0, 1000 ), true, afterburn, false )
    {
        VSRandom vsr( seed );
    ...
    void Execute()
    {
        ...
        if (targ) {
            ...
            m.SetDesiredVelocity( Vector( 0, 0, afterburn
                                         && ab_needed ? parent->GetComputerData().max_ab_speed() : parent->GetComputerData().
                                         max_speed() ), true );
...
class LoopAroundAgro : public Orders::FaceTargetITTS
{
    ...
    bool   afterburn;
    bool   force_afterburn;
public: LoopAroundAgro( bool aggressive, bool afterburn, bool force_afterburn, int seed ) : FaceTargetITTS( false, 3 )
        , m( false, 2, false )
    {
        VSRandom vsr( seed );
        this->afterburn  = afterburn;
        ...
        if (targ) {
            ...
            if (r.Dot( relloc ) < 0) {
                ...
                m.Execute( parent, targ->Position()-r.Scale(
                          ...
                          *( rr.i*parent->rSize() )+targ->cumulative_transformation_matrix.getQ()*( rr.j*parent->rSize() ) );
            } else {
                done = false;
                if (afterburn)
                    m.SetAfterburn( ab_needed );
